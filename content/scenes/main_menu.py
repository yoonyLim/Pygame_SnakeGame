...
def update():
    ...
    # initialize title
    TITLE = TextBox(gs.SCREEN_WIDTH / 2, gs.SCREEN_HEIGHT / 4, "SNAKE GAME", gs.FONT_FAMILY, gs.TITLE_FONT_SIZE, gs.TITLE_COLOR)

    # initialize buttons
    PLAY_BTN = Button(gs.SCREEN_WIDTH / 2, gs.SCREEN_HEIGHT / 2, "PLAY", gs.FONT_FAMILY, gs.BUTTON_FONT_SIZE, gs.BUTTON_DEFAULT_COLOR, 0)
    OPTIONS_BTN = Button(gs.SCREEN_WIDTH / 2, gs.SCREEN_HEIGHT / 2 + tools.getTxtRectSize("PLAY", gs.FONT_FAMILY, gs.BUTTON_FONT_SIZE)[1], "OPTIONS", gs.FONT_FAMILY, gs.BUTTON_FONT_SIZE, gs.BUTTON_DEFAULT_COLOR, 1)
    QUIT_BTN = Button(gs.SCREEN_WIDTH / 2, gs.SCREEN_HEIGHT / 2 + tools.getTxtRectSize("PLAY", gs.FONT_FAMILY, gs.BUTTON_FONT_SIZE)[1] * 2, "QUIT", gs.FONT_FAMILY, gs.BUTTON_FONT_SIZE, gs.BUTTON_DEFAULT_COLOR, 2)
    LIST_BTNS = [PLAY_BTN, OPTIONS_BTN, QUIT_BTN]
    ...
    for event in pg.event.get():
        if event.type == pg.KEYDOWN:
            ...
            if PLAY_BTN.checkForInput(gs.MOUSE_POS):
                ...
                gs.MAIN_MENU_CURRENT_OPTION_INDEX = 0
            elif OPTIONS_BTN.checkForInput(gs.MOUSE_POS):
                gs.CURRENT_SCENE_INDEX = 2
                gs.MAIN_MENU_CURRENT_OPTION_INDEX = 0
            elif QUIT_BTN.checkForInput(gs.MOUSE_POS):
                gs.CURRENT_SCENE_INDEX = 3
                gs.MAIN_MENU_CURRENT_OPTION_INDEX = 0
